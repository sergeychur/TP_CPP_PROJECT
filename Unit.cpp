...
void Unit::control(sf::Event event, sf::Vector2f cursor_pos) {
...
void Unit::update(float time)
{
    ...
    if (isMove) {
        distance = sqrt(pow((tempX - coordinates.x), 2) + pow((tempY - coordinates.y), 2));//считаем дистанцию (расстояние от точки А до точки Б). используя формулу длины вектора

        if (distance > 2) {//этим условием убираем дергание во время конечной позиции спрайта

            coordinates.x += 0.1 * time * (tempX - coordinates.x) / distance;//идем по иксу с помощью вектора нормали
