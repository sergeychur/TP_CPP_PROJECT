...
void Base::add_pop_handler(std::shared_ptr<Unit>& unit) {
    ...
    std::shared_ptr<AbstractHandler> pop_handler = std::make_shared<Handler<Unit>>(unit, "pop_command", pop_func);
    unit->add_distrib_handler(pop_handler);
}

void Base::add_check_handler(std::shared_ptr<Unit>& unit) {
    std::function<bool(Command&)> check_func = std::bind(&Unit::correct_state, unit.get(), std::placeholders::_1);
...
void Base::add_get_kicked_handler(std::shared_ptr<Unit>& unit) {
    std::function<bool(Command&)> get_kicked_func = std::bind(&Unit::get_kicked, unit.get(), std::placeholders::_1);
    std::shared_ptr<AbstractHandler> kick_handler = std::make_shared<Handler<Unit>>(unit, "get_kicked", get_kicked_func);
    unit->add_act_handler(kick_handler);
}
